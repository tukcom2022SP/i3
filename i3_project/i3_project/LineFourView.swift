...
struct LineFourView: View {
    ...
    var body: some View {
        ...
        case "토":
            List {
                ForEach (getListOfDayForFour(inputLineStore: lineStore).filter{"\($0)".contains(self.text) || text == ""}, id:\.stationCount) { line in
...
struct LineFourStationView_Previews: PreviewProvider {
    static var previews: some View {
        List {
            LineFourStationView(line: Line (
